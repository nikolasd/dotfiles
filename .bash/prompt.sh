...
function __prompt
{
    ...
    PS1="$green$user@$host $blue$wd$reset $PS1$blue\$ $reset"
    
    if [[ "$VIRTUAL_ENV" ]]; then
        # Use it if you want the project name to be shown. Actually the upper to the venv folder.
        PS1="$cyan(`basename \`dirname \"$VIRTUAL_ENV\"\``)$reset $PS1"
        # Use it if youwant the virtualenv name to be shown.
